...
- 首先，建立一個小程式，自動從網頁擷取資訊。
- 接著，建立第二個程式，用來分析這些檔案並操作其中的 metadata。
...
必須建立兩個程式，程式可以是腳本（script）或可執行檔（binary）。

- 若使用編譯型語言，必須提交完整原始碼，並在評分時完成編譯。
- 可以使用能建立 HTTP 請求與處理檔案的函式或函式庫。
- **但每個程式的核心邏輯必須自行開發。**
